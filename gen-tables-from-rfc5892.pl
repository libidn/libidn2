...
while(<>) {
    $intable = 1 if m,^Appendix B.1.,;
    $intable = 0 if m,^8.  References,;
    next if !$intable;

    next if m,^Appendix B.1.,;
    next if m,^$,;
    next if m,^$,;
    next if m,^Faltstrom                    Standards Track                   \[Page [0-9]+\]$,;
    next if m,^RFC 5892                    IDNA Code Points                 August 2010$,;
